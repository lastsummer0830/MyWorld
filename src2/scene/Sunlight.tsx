...
const FRAG = /* glsl */ `
  ...
  void main() {
    ...
    // ★ 2026-08-01(2): 살짝 낮춤. 0.62에서는 갈래 아래 놓인 에셋이 통째로 하얗게 씻겨
    //   무늬가 지워졌다(강아지 얼굴 무늬가 화면에서 사라진 게 이것 때문). depthTest가 꺼진
    //   전체화면 가산 오버레이라 밑에 뭘 두든 다 덧칠된다 — 세기는 에셋이 읽히는 선에서 잡는다.
    //   바닥값(0.02)도 함께 내려 갈래 사이를 더 비웠다. 갈래 자체의 인상은 유지된다.
    float a = (halo * 0.13 + cone * (0.012 + 0.988 * rays) * 0.46) * day;
