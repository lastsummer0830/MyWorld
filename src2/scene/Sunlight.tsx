...
const FRAG = /* glsl */ `
  ...
  /**
   * 갈래 하나의 단면. sin의 마루 꼭대기만 잘라내 **사이를 완전히 0으로** 만든다.
   *
   * pow(0.5+0.5*sin, n)으로 뾰족하게 깎는 방법도 써 봤지만(2026-08-01 1차 시도) 어깨가 넓어
   * 갈래 사이가 안 비었다. 여러 갈래의 어깨가 겹쳐 쌓이면 결국 우윳빛 막이 된다.
   * smoothstep으로 문턱을 두면 문턱 아래는 정확히 0이라 갈래 사이가 확실히 어두워진다.
   *
   * w = 마루에서 잘라낼 비율(굵기). 작을수록 가늘다.
   */
  float strand(float x, float freq, float phase, float w) {
    float s = 0.5 + 0.5 * sin(x * freq + phase);
    s = smoothstep(1.0 - w, 1.0, s);
    return s * s; //  코어를 한 번 더 세워 가운데가 밝고 가장자리가 빨리 죽게
  }
  ...
  void main() {
    ...
    //
    /**
     * ★★ 2026-08-01 교체 — 왜 갈래가 눈에 2개로만 보였나: 파동을 **곱하고** 있었다.
     *
     * 예전엔 (0.5+0.5 sin 20t) * (0.5+0.5 sin 31.4t) * (0.6+0.4 sin 11.3t) 였다. 곱은 AND다.
     * 세 파동이 **동시에** 마루일 때만 값이 크게 남고, 하나라도 골이면 통째로 죽는다. 주파수가
     * 서로 안 나누어떨어지니 세 마루가 겹치는 지점은 화면 폭 안에 두어 곳뿐 — 그래서 굵은 덩어리
     * 2개가 됐다. 곱하는 파동을 늘리거나 주파수를 올려도 겹침이 더 귀해질 뿐이라 더 나빠진다.
     *
     * → **합(OR)으로 바꾼다.** 각 파동을 pow로 뾰족하게 깎아 마루만 남긴 뒤 더하면,
     *   파동 하나의 마루가 각각 갈래 하나가 된다. 주파수·위상·굵기(pow 지수)·세기를 다르게 주면
     *   굵은 갈래 사이에 가는 갈래가 끼는 불규칙한 배열이 나온다. 지수가 클수록 가늘고 날카롭다.
     */
    // 갈래를 살짝 휘게 한다. 완전히 직선이면 종이에 자로 그은 것처럼 읽힌다.
    float tw = t + 0.045 * sin(max(along, 0.0) * 2.6 + time * 0.05);

    // ★ 주파수 = 갈래 밀도다. 섬이 차지하는 t 폭이 대략 1.0이므로 파동 하나가 만드는 갈래 수는
    //   대략 (주파수 / 6.28)개다. 처음 12/20/29/44로 잡았더니 섬 위에 17갈래가 들어가 서로 뭉개져
    //   다시 뿌연 막이 됐다 — 합으로 바꿔도 너무 촘촘하면 갈래로 안 읽힌다. 총 7~8갈래가 적당하다.
    //   (※ 예전 "주파수를 낮추지 말 것" 경고는 파동을 **곱하던** 시절 얘기다. 곱에서는 주파수를
    //    낮추면 겹침 지점이 더 귀해져 덩어리가 됐다. 합에서는 반대로 갈래가 굵고 또렷해진다.)
    // 굵기·간격이 다른 갈래 세 벌. 더하지 않고 max로 겹친다 — 더하면 두 갈래가 스칠 때마다
    // 그 자리만 두 배로 밝아져 얼룩이 생기고, 어깨끼리 쌓여 배경이 다시 뜬다.
    float rays = max(max(1.00 * strand(tw,  9.4, 0.0 + time * 0.050, 0.26),   // 굵은 갈래
                         0.74 * strand(tw, 15.1, 1.9 - time * 0.040, 0.17)),
                         0.48 * strand(tw, 24.3, 3.4 + time * 0.030, 0.11));  // 가는 갈래

    // 갈래 세기를 고르게 두면 빗살처럼 규칙적으로 보인다. 아주 느린 파동을 곱해 어떤 갈래는
    // 밝고 어떤 갈래는 흐리게 — 무리를 지어 들어오는 인상을 만든다.
    rays *= 0.28 + 0.72 * (0.5 + 0.5 * sin(tw * 3.7 - 0.8 + time * 0.02));
    ...
    // ★ 2026-08-01: rays를 곱 → 합으로 바꾸면서 평균값이 크게 올랐다(곱일 땐 대부분 0에 가까웠다).
    //   계수를 그대로 두면 원뿔 전체가 우윳빛으로 씻긴다. 0.62 → 0.30으로 낮추고
    //   바닥값도 0.05 → 0.03으로 내려 갈래 사이를 더 비운다.
    float a = (halo * 0.16 + cone * (0.02 + 0.98 * rays) * 0.62) * day;
