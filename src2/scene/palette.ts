...
export const COLOR = {
  ...
  // 생물 (단계 4)
  // ── 강아지 (조아진의 실제 반려견 사진 기준으로 2026-08-01 교체) ──
  // ★ 예전 값은 "크림색 몸 + 회청색 무늬"였는데 사진과 다르다. 실제로는:
  //   흰색이 주조색(가슴 러프·앞다리·배·꼬리), 등판만 은회색에 검은 얼룩(merle),
  //   얼굴은 검정 바탕에 눈두덩 탄 포인트 + 주둥이에서 이마로 올라가는 흰 블레이즈.
  //   누런 크림으로 칠하면 셸티가 아니라 골든 계열로 읽힌다.
  dogWhite: '#F6F4EF', //  주조색 — 가슴 러프·다리·배·꼬리
  dogSilver: '#AFB6BD', //  등판 은회색
  dogMerle: '#5A6068', //  등판의 검은 얼룩
  dogCharcoal: '#33373D', //  얼굴·귀의 검정
  dogTan: '#C08A4E', //  눈두덩·볼의 황갈색 포인트
  dogCream: '#F2E3C8', //  (구) 발바닥 등 따뜻한 소품에만 남겨 둔다
  dogBrown: '#B98A5F',
  dogSlate: '#8C93A6',
  dogInk: '#24262B', //  코·눈동자 — 사진의 코는 거의 검정이다
  eyeBlue: '#7FA9C9', //  오드아이의 파란 눈
